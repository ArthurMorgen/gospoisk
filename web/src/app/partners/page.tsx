  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-16">
        ...
        {/* Как это работает */}
        <div className="mb-14">
          <h2 className="mb-8 text-center text-lg font-bold text-zinc-900">Как это работает</h2>
          <div className="grid gap-5 sm:grid-cols-3">
            {STEPS.map((step, i) => (
              <div key={i} className="rounded-2xl border border-zinc-100 p-6 transition-all hover:border-zinc-200 hover:shadow-md">
                <div className={`mb-4 flex h-10 w-10 items-center justify-center rounded-xl ${step.color}`}>
                  {step.icon}
                </div>
                <h3 className="mb-2 text-sm font-bold text-zinc-900">{step.title}</h3>
        ...
        {/* Условия */}
        <div className="mb-14">
          <h2 className="mb-6 text-center text-lg font-bold text-zinc-900">Сколько можно заработать</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            {CONDITIONS.map((c) => (
              <div key={c.label} className="rounded-2xl border border-zinc-100 bg-zinc-50/50 p-6">
                <p className="mb-1 text-sm text-zinc-500">{c.label}</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-extrabold text-zinc-900">{c.reward}</span>
        ...
        <div className="mb-14">
          <h2 className="mb-6 text-center text-lg font-bold text-zinc-900">Кто может стать партнёром</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={item} className="flex items-start gap-3 rounded-xl border border-zinc-100 p-4">
